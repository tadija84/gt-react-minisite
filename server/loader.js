...
import { setCurrentUser, logoutUser, setIsMobile } from "../src/modules/auth";
  ...
  */
  ...
  fs.readFile(
    ...
    (err, htmlData) => {
      ...
      store.dispatch(setIsMobile(isMobile === undefined || !isMobile ? false : true));
      /*
      ...
      */
      frontloadServerRender(() =>
        ...
        if (context.url) {
          ...
          Preloader(req, res).then(response => {
            var meta = response.data;
            var title = "Vazno gde kupujete tehniku | Gigatron";
            if (meta.prefix) {
              title = "Gigatron - " + meta.title;
            } else if (meta.sufix) {
              title = meta.title + " | Gigatron";
            } else {
              title = meta.title;
            }

            helmet.title = `<title>${title}</title>`;
            helmet.meta = `<meta name="og:title" content="${title}" /><meta name="og:description" content="${
              meta.description
